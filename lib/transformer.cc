...
// Absen C9 1/8 scan transformer
class DoubleAbsenTransformer::TransformCanvas : public Canvas {
public:
  TransformCanvas();
...
DoubleAbsenTransformer::TransformCanvas::TransformCanvas()
  : delegatee_(NULL) {
  }

void DoubleAbsenTransformer::TransformCanvas::SetDelegatee(Canvas* delegatee) {
  delegatee_ = delegatee;
}

void DoubleAbsenTransformer::TransformCanvas::SetPixel(int x, int y, uint8_t red, uint8_t green, uint8_t blue) {
  ...
  mapped_x = mapping[x%64][y%16][0] + x - (x%64);
  mapped_y = mapping[x%64][y%16][1] + y - (y%16);
  delegatee_->SetPixel(mapped_x, mapped_y, red, green, blue);
}

int DoubleAbsenTransformer::TransformCanvas::width() const {
  return delegatee_->width();
}

int DoubleAbsenTransformer::TransformCanvas::height() const {
  return delegatee_->height();
}

void DoubleAbsenTransformer::TransformCanvas::Clear() {
  delegatee_->Clear();
}

void DoubleAbsenTransformer::TransformCanvas::Fill(uint8_t red, uint8_t green, uint8_t blue) {
  delegatee_->Fill(red, green, blue);
}


DoubleAbsenTransformer::DoubleAbsenTransformer()
...
Canvas *DoubleAbsenTransformer::Transform(Canvas *output) {
  assert(output != NULL);
